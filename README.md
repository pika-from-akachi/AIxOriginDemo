...
| ④ OLED 交互 | U8g2 128x64：状态栏 + 大字等级 + 存活率 + 逃生箭头/文案 | 高危整屏反白闪烁 + 可选告警灯 |
...
| `OLED_MODEL` | `1` | 1=SSD1306  2=SH1106  3=SSD1309 |
| `OLED_SDA` / `OLED_SCL` | `8` / `9` | ESP32-S3 DevKitC 默认 I2C 引脚 |
| `OLED_I2C_ADDR` | `0x3C*2` | 屏幕 I2C 地址(8bit) |
...
├──────────────────────────────┤
│ DANGER          SURVIVAL     │  ← 左: 大字危险等级;  右: 存活率
│ MUDSLIDE R97       25 %      │     左第二行: 灾害类型+危险指数
│  ▲                         │
│  EVACUATE FLOOD              │  ← 底部: 逃生指引两行
│  GO HIGH GROUND              │
└──────────────────────────────┘
```

- **安全(绿)**：`SAFE`，白底黑字，底部 `ALL CLEAR`。
- **预警(黄)**：`WARN`，静态界面，提示向高处移动。
- **危险(红)**：`DANGER`，整屏反白 350ms 闪烁（可外接告警灯同步闪烁）。
